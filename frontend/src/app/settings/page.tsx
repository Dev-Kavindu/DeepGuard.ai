...
export default function SettingsPage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const { error: uploadError } = await supabase.storage
      .from("incident_vault")
      .upload(filePath, file, { cacheControl: "3600", upsert: true, contentType: file.type || "video/mp4" });
    ...
    const { data: publicUrlData } = supabase.storage.from("incident_vault").getPublicUrl(filePath);
  ...
  const confirmDelete = async () => {
    ...
    const { error } = await supabase.from("cameras").delete().eq("id", camToDelete);
    if (!error) {
      setCameras(cameras.filter((cam) => cam.id !== camToDelete));
    }
    setCamToDelete(null);
  };
    ...
    <div className="relative space-y-7">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight sm:text-3xl">Camera Configuration</h1>
          <p className="mt-1 text-sm text-zinc-500">Manage live RTSP streams or test MP4 videos.</p>
        </div>
        <button
          type="button"
        ...
        <div className="lg:col-span-2 space-y-4">
          {error ? (
            <div className="glass-panel flex min-h-56 flex-col items-center justify-center border-dashed p-10 text-center">
              ...
              <p className="text-sm font-medium text-zinc-200">Camera configuration unavailable</p>
            ...
            <div className="glass-panel p-10 flex flex-col items-center justify-center text-zinc-500 border-dashed">
              ...
              <p>No cameras configured yet. Add an RTSP stream or Test Video URL.</p>
            </div>
          ) : (
            cameras.map((cam) => (
              <div key={cam.id} className="glass-panel group flex flex-col gap-4 p-5 smooth-transition hover:border-zinc-700">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3">
                    <div className="rounded-lg bg-zinc-800 p-2 group-hover:bg-emerald-400/10 group-hover:text-emerald-400 smooth-transition">
                      <Camera size={20} className="text-zinc-300 group-hover:text-emerald-400" />
                    </div>
                    <div>
                      <h3 className="font-semibold text-lg">{cam.name}</h3>
                      <p className="mt-1 max-w-[200px] truncate font-mono text-xs text-zinc-500 sm:max-w-xs" title={cam.stream_url}>
                        {cam.stream_url}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={`rounded-full border px-2 py-1 text-[10px] font-bold uppercase tracking-wider ${cam.status === "active" ? "border-emerald-500/20 bg-emerald-500/10 text-emerald-400" : "border-zinc-700 bg-zinc-800 text-zinc-400"}`}>
                      {cam.status || "Active"}
                    </span>
                    <button
                      type="button"
                      onClick={() => setCamToDelete(cam.id)}
                ...
                <div className="bg-zinc-950/50 p-4 rounded-xl border border-zinc-800/50">
                  <div className="flex justify-between items-center mb-2">
                    <label htmlFor={`threshold-${cam.id}`} className="text-sm font-medium flex items-center gap-2 text-zinc-300">
                  ...
                  <input
                    ...
                    onChange={(e) => handleThresholdChange(cam.id, parseInt(e.target.value, 10))}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="relative max-h-[90vh] w-[92vw] max-w-lg overflow-y-auto rounded-2xl border border-zinc-700 bg-zinc-900 p-4 shadow-2xl sm:p-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-zinc-400 mb-1">Camera Name / Location</label>
                <input
                  type="text"
              ...
              <div>
                ...
                <div className="flex flex-col items-start gap-3 rounded-lg border border-dashed border-zinc-800/50 bg-zinc-950/50 p-3 sm:flex-row sm:items-center">
                  <input type="file" accept="video/mp4" onChange={handleFileUpload} className="hidden" id="video-upload" />
                  <label htmlFor="video-upload" className="px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-xs font-medium rounded-md cursor-pointer smooth-transition flex items-center gap-2">
                    {isUploading ? <span className="animate-spin h-3 w-3 border-b-2 border-white rounded-full"></span> : <Video size={14} />}
                    {isUploading ? "Uploading..." : "Upload Test MP4"}
                  </label>
                  <span className="text-xs text-zinc-400">{uploadStatus || "Auto-generates the URL after upload."}</span>
                </div>
              </div>
              <div>
                <div className="flex justify-between items-center mb-1">
                  <label className="block text-sm font-medium text-zinc-400">Default AI Threshold</label>
                  <span className="text-sm font-bold text-blue-400">{newCam.threshold}%</span>
                </div>
                <input
                  type="range"
                  min="10"
                  ...
                  onChange={(e) => setNewCam({ ...newCam, threshold: parseInt(e.target.value, 10) })}
                  className="w-full h-2 bg-zinc-800 rounded-lg appearance-none cursor-pointer accent-blue-500"
                />
              </div>
              <div className="flex flex-col-reverse gap-2 pt-4 sm:flex-row sm:justify-end">
                <button onClick={() => setIsModalOpen(false)} className="w-full rounded-lg bg-zinc-800 py-2 text-sm font-medium smooth-transition hover:bg-zinc-700 sm:w-auto sm:px-5">
                  Cancel
                </button>
                <button
                  onClick={handleAddCamera}
                  disabled={!newCam.name || !newCam.url || isSaving || isUploading}
                ...
                >
                  {isSaving ? <span className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></span> : "Add Camera"}
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm rounded-2xl border border-zinc-700 bg-zinc-900 p-6 shadow-2xl">
            ...
            <p className="text-sm text-zinc-400 mb-6">
              Are you sure you want to delete this camera from the system? The AI will stop monitoring this feed.
            </p>
            <div className="flex justify-end gap-3">
              <button onClick={() => setCamToDelete(null)} className="px-4 py-2 rounded-lg bg-zinc-800 text-sm font-medium hover:bg-zinc-700 smooth-transition">
                Cancel
              </button>
              <button onClick={confirmDelete} className="px-4 py-2 rounded-lg bg-red-600 text-sm font-medium text-white hover:bg-red-700 smooth-transition">
                Delete
              </button>
            </div>
          </div>
