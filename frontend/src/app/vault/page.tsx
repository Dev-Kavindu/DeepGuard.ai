...
export default function VaultPage() {
  ...
  // වීඩියෝව ප්ලේ කිරීම සඳහා Modal State
  const [selectedVideo, setSelectedVideo] = useState<string | null>(null);

  // 1. Database එකෙන් Incidents සහ ඊට අදාළ කැමරාවේ නම ලබාගැනීම
  ...
  // 2. False Alarm ලෙස ලකුණු කිරීම (Human-in-the-loop AI Feedback)
  const handleFalseAlarm = async (id: number) => {
    ...
    if (!error) {
      setIncidents(
        incidents.map((inc) =>
          inc.id === id ? { ...inc, is_false_alarm: true } : inc,
        ),
      );
    }
  };

  // 3. Incident එක මකා දැමීම
  const handleDelete = async (id: number) => {
    if (!window.confirm("Delete this incident record and video evidence?"))
      return;
  ...
  return (
    <div className="relative space-y-7">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500">
            Review and manage AI-detected security events.
          </p>
        </div>
        <button
          type="button"
          className="flex w-fit items-center gap-2 rounded-lg border border-zinc-700 bg-zinc-800 px-4 py-2 text-sm text-white smooth-transition hover:bg-zinc-700"
        >
          <Calendar size={18} />
      ...
      <div className="glass-panel flex flex-col items-stretch gap-3 p-3 sm:flex-row sm:items-center sm:p-4">
        ...
        <button
          type="button"
          className="flex items-center justify-center gap-2 rounded-lg border border-zinc-800 bg-zinc-950 px-4 py-2.5 text-sm hover:border-zinc-600 smooth-transition"
        >
          <Filter size={18} />
          <span>Filter</span>
        </button>
            ...
            <tbody className="divide-y divide-zinc-800/50">
              {isLoading ? (
                <tr>
                  ...
                  >
                    ...
                    <span className="mt-1 block text-xs text-zinc-600">
                      Decrypting recent evidence records
                    </span>
                  </td>
                </tr>
                ...
                filteredIncidents.map((incident) => (
                  <tr
                    key={incident.id}
                    className={`hover:bg-zinc-800/20 smooth-transition ${incident.is_false_alarm ? "opacity-50 grayscale" : ""}`}
                  >
                    <td className="px-6 py-4 font-mono text-zinc-300">
                      INC-{incident.id.toString().padStart(4, "0")}
                    </td>
                    <td className="px-6 py-4">
                      {incident.cameras?.name || "Unknown Camera"}
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-1 items-start">
                        <span
                          className={`font-semibold ${incident.is_false_alarm ? "text-zinc-500 line-through" : "text-zinc-200"}`}
                        >
                        ...
                        {!incident.is_false_alarm && (
                          <span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-amber-500/10 text-amber-400 border border-amber-500/20">
                            Possible: {incident.anomaly_type}
                          </span>
                        )}
                      </div>
                    </td>
                      ...
                      <div className="flex justify-end gap-3">
                        {!incident.is_false_alarm && (
                          <button
                            ...
                            className="text-zinc-400 hover:text-amber-400 smooth-transition"
                        ...
                        <button
                          type="button"
                          onClick={() => setSelectedVideo(incident.video_url)}
                        ...
                        >
                          <Play
                            size={18}
                            className={!incident.video_url ? "opacity-30" : ""}
                          />
                        </button>
                        <a
                          ...
                          download
                          target="_blank"
                          className={`text-zinc-400 hover:text-green-400 smooth-transition ${!incident.video_url ? "pointer-events-none opacity-30" : ""}`}
