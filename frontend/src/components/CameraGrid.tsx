  ...
  useEffect(() => {
    const fetchCameras = async () => {
      ...
      if (data) {
        ...
        setError(
          "Camera network is unavailable. Check your connection and try again.",
        );
      }
      setIsLoading(false);
    };
      ...
      {isLoading ? (
        <div className="col-span-full flex min-h-[360px] flex-col items-center justify-center text-zinc-500">
          ...
          <span className="mt-1 text-xs text-zinc-600">
            Establishing secure live channels...
          </span>
        </div>
      ) : error ? (
        ...
        <div className="col-span-full flex min-h-[360px] flex-col items-center justify-center text-zinc-500">
          ...
          <span className="mt-1 text-xs text-zinc-600">
            Add a camera in Settings to begin monitoring.
          </span>
        </div>
      ) : (
        cameras.map((cam) => (
          ...
          >
              ...
              {cam.isAlert ? (
                ...
                <div className="flex items-center gap-1">
                  {cam.isActive ? (
                    ...
                    <span className="text-[10px] px-2 py-1 rounded-full bg-red-500/10 border border-red-500/20 text-red-400 font-bold uppercase tracking-wider">
                      Offline
                    </span>
                  )}
                </div>
            ...
            {/* Bottom Overlay - Time & Watermark (For Evidence) */}
            <div className="absolute bottom-0 z-10 flex w-full items-end justify-between bg-gradient-to-t from-black/90 via-black/50 to-transparent p-3">
              ...
              <span className="text-xs font-bold text-blue-500/70 tracking-widest drop-shadow-md">
                DeepGuard.ai
              </span>
            </div>
          </div>
